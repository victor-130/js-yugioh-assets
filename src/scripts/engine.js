...
async function getRandomCardId() {
    const randemIndex = Math.floor(Math.random() * cardData.length)
    return cardData[randemIndex].id
}
...
async function setCardsFields(cardId) {
    ...
    drawCardsInFields(cardId,computerCardId)
...
async function checkDualResults(playerCardId, computerCardId) {
    ...
    let platerCard = cardData[playerCardId]

    if(platerCard.WinOf.includes(computerCardId)){
        duelResults = "Win"
        
        state.score.playerScore++
    }
    if(platerCard.LoseOf.includes(computerCardId)){
        duelResults = "Lose"
